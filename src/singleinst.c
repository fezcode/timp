...
static const char *sock_path(void) {
    if (!g_sock_path[0]) {
        ...
        snprintf(g_sock_path, sizeof(g_sock_path), "%s/.timp", home);
        mkdir(g_sock_path, 0755);
        size_t n = strlen(g_sock_path);
        snprintf(g_sock_path + n, sizeof(g_sock_path) - n, "/timp.sock");
    }
    return g_sock_path;
}
...
bool singleinst_acquire(void) {
    // A live socket means a running Timp; a dead file is a stale crash leftover.
